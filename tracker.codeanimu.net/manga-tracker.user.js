...
var sites = {
	'mangafox.me' : function() {
		...
		$('script[src*="js/list"]').on("load", function() {
			...
			$(window).on("load", function() {
				...
				if(config.auto_track && config.auto_track == 'on') {
					if(config['api-key']) {
						...
						$.post(main_site + '/ajax/update_tracker', json, function (data) {
							console.log(data);
						});
			...
			setupTopBar(chapterList, manga_url + volume+'/'+chapter, function(topBar) {
...
function setupTrackerOptions() {
	...
	$('#loading-userscript').replaceWith(function() {
		var form = $('<form/>', {id: 'userscript-config'}).append(
			$('<h3/>', {text: 'General Options'})).append(
			$('<div/>', {class: 'form-group'}).append(
				$('<label/>', {'for': 'api-key', 'text': 'API Key '})).append(
				$('<input/>', {id: 'api-key', name: 'api-key', type: 'text', value: config['api-key'], 'readonly': '1', style: 'width: 250px; margin: auto 3px; background-color: rgb(255, 255, 255); border-width: 2px; border-style: inset; text-align: center;'})).append(
				$('<a/>', {id: 'generate-api-key', href: '#', text: 'Generate new API key'}))).append(
			$('<div/>', {class: 'form-group'}).append(
				$('<label/>', {'for': 'auto_track', 'text': 'Auto track series on page load '}).append(
				$('<input/>', {id: 'auto_track', name: 'auto_track', type: 'checkbox', checked: !!config['auto_track']})))).append(
		...
		$(form).on('click', '#generate-api-key', function() {
			$.getJSON(main_site + '/ajax/get_apikey', function(json) {
				if(json['api-key']) {
					$('#api-key').val(json['api-key']);
				}
			});
		});
